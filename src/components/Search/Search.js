...
const Search = props => {
  const search = evt => {
    if (evt.key === "Enter") {
      axios.get(`${process.env.REACT_APP_API_BASE}weather?q=${props.query}&units=metric&appid=${process.env.REACT_APP_API_KEY}`)
      .then(result => {
      ...
      axios.get(`${process.env.REACT_APP_API_BASE}forecast?q=${props.query}&units=metric&appid=${process.env.REACT_APP_API_KEY}`)
      .then(result => {
